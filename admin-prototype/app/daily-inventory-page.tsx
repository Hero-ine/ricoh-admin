...
export function DailyInventoryPage() {
  ...
  return <>
    ...
    {error ? <p role="alert" className="form-error">{error}</p> : <div className="table-scroll"><Table aria-label="每日库存更新" className="recipient-data-table daily-inventory-table"><TableHeader><TableColumn isRowHeader>日期</TableColumn><TableColumn>GR4</TableColumn><TableColumn>GR3x</TableColumn><TableColumn>GR3x HDF</TableColumn><TableColumn>会员卡</TableColumn></TableHeader><TableBody renderEmptyState={()=>loading?'正在加载':'暂无每日统计'}>{rows.map(row=><TableRow key={row.date} id={row.date}><TableCell><strong>{row.date}</strong>{row.incomplete&&<small>数据不完整</small>}</TableCell>{columns.map(id=><TableCell key={id}>{row.values[id]??'—'}</TableCell>)}</TableRow>)}</TableBody></Table></div>}
  </>;
}
