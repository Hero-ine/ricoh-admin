...
export async function requireAdmin(request?: Request) { if (!await isAuthenticated()) return Response.json({ error: '请先登录' }, { status: 401 }); if (request && request.method !== 'GET') { const origin = (await headers()).get('origin'); if (origin && origin !== new URL(request.url).origin) return Response.json({ error: '请求来源无效' }, { status: 403 }); } return null; }
